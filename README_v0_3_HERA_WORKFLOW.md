...
try:
    from skyfield.api import EarthSatellite as skyfield_EarthSatellite
    from skyfield.api import load as skyfield_load, wgs84 as skyfield_wgs84
except Exception as exc:  # pragma: no cover
    skyfield_EarthSatellite = None
    skyfield_load = None
    skyfield_wgs84 = None
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--reference-flux-jy", type=float, default=None, help="Override starlink.reference_flux_jy / flux_jy_ref.")
...
def _tle_norad_id(line1: str) -> str:
...
def _tle_international_designator(line1: str) -> str:
    return _tle_field(line1, 9, 17)


def _tle_dedupe_key(line1: str, line2: str) -> Tuple[str, str, str]:
    # Ignore final checksum columns; historical Starlink TLE dumps sometimes repeat
    # the same orbital record with only checksum/formatting differences.
    return (_tle_norad_id(line1), _tle_epoch(line1), line2[:68].strip())


def _tle_fallback_name(line1: str) -> str:
    norad = _tle_norad_id(line1) or "UNKNOWN"
    epoch = _tle_epoch(line1) or "NOEPOCH"
    return f"STARLINK-{norad}-E{epoch}"


def satellite_display_name(sat: Any, fallback_index: int | None = None) -> str:
    name = str(getattr(sat, "name", "") or "").strip()
    if name:
        return name
    norad = str(getattr(sat, "_ow_tle_norad_id", "") or "").strip()
    epoch = str(getattr(sat, "_ow_tle_epoch", "") or "").strip()
    if norad or epoch:
        return f"STARLINK-{norad or 'UNKNOWN'}-E{epoch or 'NOEPOCH'}"
    if fallback_index is not None:
        return f"sat_{int(fallback_index)}"
    return "unknown"


def load_tle_satellites(
    tle_path: Path,
    max_scan_satellites: Optional[int] = None,
    target_jd: Optional[float] = None,
) -> Tuple[List[Any], Dict[str, Any]]:
    """Load TLEs with deterministic names and one orbital record per NORAD ID.

    Starlink GP history files often contain many TLE epochs for the same NORAD
    object and may omit name lines. Treating those records as independent
    satellites artificially inflates the visible-emitter count, so this loader
    collapses exact record duplicates and then keeps one record per NORAD ID.
    If target_jd is supplied, the selected record is closest to that time;
    otherwise the latest epoch in the file is used.
    """
    if skyfield_load is None or skyfield_EarthSatellite is None:
        raise ImportError(f"skyfield unavailable: {SKYFIELD_ERROR}")
    ts = skyfield_load.timescale()
    raw_lines = [line.strip() for line in Path(tle_path).read_text(encoding="utf-8").splitlines() if line.strip()]
    ...
    while idx < len(raw_lines):
        name = ""
        if raw_lines[idx].startswith("1 ") and idx + 1 < len(raw_lines) and raw_lines[idx + 1].startswith("2 "):
            line1 = raw_lines[idx]
            line2 = raw_lines[idx + 1]
            idx += 2
        elif (
            idx + 2 < len(raw_lines)
            and raw_lines[idx + 1].startswith("1 ")
            and raw_lines[idx + 2].startswith("2 ")
        ):
            name = raw_lines[idx].strip()
            line1 = raw_lines[idx + 1]
            line2 = raw_lines[idx + 2]
            idx += 3
        else:
            idx += 1
            continue
        ...
        total_records += 1
        key = _tle_dedupe_key(line1, line2)
        if key in seen:
            exact_duplicates_removed += 1
            continue
        seen.add(key)
        sat_name = name or _tle_fallback_name(line1)
        sat = skyfield_EarthSatellite(line1, line2, sat_name, ts)
        unique_records.append({
            "sat": sat,
            "name": sat_name,
            "norad_id": _tle_norad_id(line1),
            "epoch": _tle_epoch(line1),
            "international_designator": _tle_international_designator(line1),
            "identity": "|".join(key),
            "original_name_blank": not bool(name),
            "epoch_jd": float(sat.epoch.tt),
        })

    grouped: Dict[str, Dict[str, Any]] = {}
    for rec in unique_records:
        norad = rec["norad_id"] or rec["identity"]
        old = grouped.get(norad)
        if old is None:
            grouped[norad] = rec
            continue
        if target_jd is None:
            take_new = float(rec["epoch_jd"]) > float(old["epoch_jd"])
        else:
            take_new = abs(float(rec["epoch_jd"]) - float(target_jd)) < abs(float(old["epoch_jd"]) - float(target_jd))
        if take_new:
            grouped[norad] = rec

    selected_records = sorted(grouped.values(), key=lambda rec: (rec["norad_id"], rec["epoch"]))
    if max_scan_satellites is not None:
        selected_records = selected_records[: int(max_scan_satellites)]

    sats: List[Any] = []
    for rec in selected_records:
        sat = rec["sat"]
        setattr(sat, "_ow_tle_norad_id", rec["norad_id"])
        setattr(sat, "_ow_tle_epoch", rec["epoch"])
        setattr(sat, "_ow_tle_international_designator", rec["international_designator"])
        setattr(sat, "_ow_tle_identity", rec["identity"])
        setattr(sat, "_ow_tle_original_name_blank", rec["original_name_blank"])
        setattr(sat, "_ow_tle_selection", "nearest_epoch_to_window" if target_jd is not None else "latest_epoch_per_norad")
        sats.append(sat)

    return sats, {
        "tle_records_total": int(total_records),
        "tle_unique_tle_records": int(len(unique_records)),
        "tle_unique_records": int(len(sats)),
        "tle_exact_duplicates_removed": int(exact_duplicates_removed),
        "tle_history_records_collapsed": int(max(len(unique_records) - len(grouped), 0)),
        "tle_duplicates_removed": int(exact_duplicates_removed + max(len(unique_records) - len(grouped), 0)),
        "tle_name_line_present": bool(any(not getattr(s, "_ow_tle_original_name_blank", True) for s in sats)),
        "tle_synthetic_names_used": bool(any(getattr(s, "_ow_tle_original_name_blank", False) for s in sats)),
        "tle_selection_strategy": "nearest_epoch_to_window" if target_jd is not None else "latest_epoch_per_norad",
    }
...
def select_satellite_from_tle(tle_path: Path, cfg: Dict[str, Any], times_jd: np.ndarray, site_cfg: Dict[str, Any]):
    ...
    sats, tle_meta = load_tle_satellites(
        tle_path,
        max_scan_satellites=sat_cfg.get("max_scan_satellites"),
        target_jd=float(np.nanmedian(times_jd)),
    )
    if not sats:
    ...
    if name:
        for s in sats:
            sname = satellite_display_name(s)
            if sname == name or name.upper() in sname.upper():
                return s, {"satellite_name": sname, "selection": "name", **tle_meta}
    ...
    if not rows:
        ...
        return sat, {
            "satellite_name": satellite_display_name(sat, idx),
            "satellite_index": idx,
...
def build_starlink_visibility(ctx: BackgroundContext, cfg: Dict[str, Any], out_dir: Path, sat_override=None, selected_override: Optional[Dict[str, Any]] = None, random_seed_offset: int = 0) -> Tuple[np.ndarray, pd.DataFrame, Dict[str, Any], Dict[str, np.ndarray]]:
    ...
    if sat_override is None:
        ...
        selected.setdefault("satellite_name", satellite_display_name(sat))
...
def enumerate_window_satellites(
    ...
    sats, tle_meta = load_tle_satellites(
        tle_path,
        max_scan_satellites=int(max_scan_satellites),
        target_jd=float(np.nanmedian(times_jd)),
    )
    if not sats:
    ...
    for wid in sorted(np.unique(window_idx)):
        ...
        for sidx, sat in enumerate(sats):
            ...
            entries.append((
                sat,
                {
                    "satellite_name": satellite_display_name(sat, sidx),
                    "satellite_index": int(sidx),
                    "norad_id": getattr(sat, "_ow_tle_norad_id", ""),
                    "tle_epoch": getattr(sat, "_ow_tle_epoch", ""),
                    "tle_international_designator": getattr(sat, "_ow_tle_international_designator", ""),
                    "tle_identity": getattr(sat, "_ow_tle_identity", ""),
                    "window_idx": int(wid),
    ...
    for wid, win in windows.items():
        ...
        if not sat_entries:
            all_window_rows.append({
                ...
                "tle_records_total": win.get("tle_records_total"),
                "tle_unique_tle_records": win.get("tle_unique_tle_records"),
                "tle_unique_records": win.get("tle_unique_records"),
                "tle_exact_duplicates_removed": win.get("tle_exact_duplicates_removed"),
                "tle_history_records_collapsed": win.get("tle_history_records_collapsed"),
                "tle_duplicates_removed": win.get("tle_duplicates_removed"),
                "tle_selection_strategy": win.get("tle_selection_strategy"),
                "tle_synthetic_names_used": win.get("tle_synthetic_names_used"),
                "selected_pol": selected_polarization_product(ctx, cfg),
        ...
        for rep in per_reports:
            flat = {
                ...
                "norad_id": rep.get("selected_satellite", {}).get("norad_id"),
                "tle_epoch": rep.get("selected_satellite", {}).get("tle_epoch"),
                "tle_international_designator": rep.get("selected_satellite", {}).get("tle_international_designator"),
                "tle_identity": rep.get("selected_satellite", {}).get("tle_identity"),
...
def main() -> None:
    ...
    if args.reference_flux_jy is not None:
        cfg.setdefault("starlink", {})["reference_flux_jy"] = float(args.reference_flux_jy)
        cfg.setdefault("starlink", {})["flux_jy_ref"] = float(args.reference_flux_jy)
